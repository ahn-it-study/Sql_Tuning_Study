...
순간적인 눈동적으로 시작지점을 바로 찾을 수 있었을 것이다.(수직적 탐색)  
...
그래서 옵티마이저는 자동 형변환을 시도했고 결과적으로 Range Scan을 할 수없었기 때문이다.  
TO_NUMBER(생년월일) = 20000507  
- 숫자와 문자형이 문나면 숫자형이 우선순위가 높음
...
### 결론
- 자동 형변환에 의존하지 말고 인덱스 컬럼 기준으로 반대편 컬럼 또는 값을 정확하게(명시적으로) 형변해줘야한다.  
