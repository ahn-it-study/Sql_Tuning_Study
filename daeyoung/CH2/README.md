...
# 인덱스 확장기능 사용법
인덱스 스캔 방식에는 Index Range Scan, Index Full Scan 말고도 다른 스캔방식이 있다.  
- Index Range Scan
- Index Full Scan
- Index Unique Scan
- Index Skip Scan
## Index Range Scan
루트에서 리프 블록까지 수직적으로 탐색 후 필요한 범위만 스캔한다.  
-> 참고로 선두컬럼이 가공되지 않아야 한다.
-> 인덱스 탄다고 성능이 좋은게 아님  
-> **성능은 인덱스 스캔 범위, 테이블 액세스 횟수를 얼마나 줄일 수 있냐로 결정된다**
## Index Full Scan
풀스캔인 범위 탐색과 달리 리프블록을 처음부터 끝까지 수평적으로 탐색하는 방법이다.  
-> 대개 데이터 검색을 위한 최적의 인덱스가 없을 때 차선으로 선택된다.
### Index Full Scan의 효용성
인덱스 선두 컬럼이 조건절에 없으면 옵티마이저는 먼저 Table Full Scan을 고려한다.  
데이터 저장공간은 가로 * 세로 즉, 컬럼 길이 * 레코드 수에 의해 결정되므로 인덱스가 차지하는 면적은 테이블보다 훨씬 적다.  
Index Range Scan을 할 수 없을 때 테이블 전체를 스캔하기보다 Index 전체를 스캔하는쪽이 유리하다.  
-> 인덱스 스캔 단계에서 대부분 레코드를 필터링하고 아주 일부분만 테이블을 액세스 하는 상황이라면, 면적이 큰 테이블보다 인덱스를  
스캔하는 쪽이 유리  
```sql
SELECT * FROM EMP WHERE SAL > 9000 ORDER BY ename;
```
![위 조건을 만족할 때 인덱스 풀스캔](https://user-images.githubusercontent.com/23313008/164964943-bd53a258-4976-4370-9d76-804f8cc90a22.png)  
sal > 9000인 사원이 위 그림과 같이 극히 일부라면 Table Full Scan보다 Index Full Scan을 통한 필터링이 효과적  
수행 빈도가 낮은 SQL이면 상관없지만 그렇지 않다면 찾으려는 자주 조건으로 들어가는 컬럼이 선두인 인덱스로 생성해주는것이 좋다.  
### Index를 사용한 소트 연산 생략
인덱스를 Full Scan하면 Range Scan과 마찬가지로 결과집합이 인덱스 컬럼 순으로 정렬된다.  
만약 대부분의 사원이 SAL > 1000을 만족하는 상황에서 Index Full Scan을 선택하면 거의 모든 레코드에 대해 테이블 액세스  
가 발생하므로 Table Full Scan보다 오히려 불리함.  
### Index Unique Scan
Index Unique Scan은 수직적 탐색만으로 데이터를 찾는 스캔 방식으로서 Unique 인덱스를 '=' 조건으로 탐색하는 경우에 작동한다.  
- Unique Index는 컬럼이 중복되지 않게 DBMS가 데이터 정합성을 관리해준다.
따라서 **인덱스 키 컬럼을 모두 '='조건으로 검색할 때는 데이터를 한건 찾는 순간 더이상 탐색할필요 없다.**  
### Unique Scan검색이 Index Range Scan으로 탐색하는 경우
Unique 인덱스라고 해도 범위검색 조건(between, 부등호, like)로 검색할 땐 Index Range Scan으로 처리된다.  
왜냐하면 수직적 탐색만으로는 해당 조건에 맞는 레코드를 찾을 수 없기 떄문이다.  
Unique 결합 인덱스의 경우 일부컬럼만 검색할 때도 Index Range Scan이 일어난다.  
### Index Skip Scan
...
- **인덱스 선두 컬럼의 Distinct Value 갯수가 적고 후행 컬럼의 Distinct Value개수가 많을 때 유용하다.**
- Index Skip Scan은 루트 또는 브랜치 블록에서 읽은 컬럼 값 정보를 이용해 조건절에 부합하는 레코드를  
  포함할 가능성이 있는 리프 블록만 골라서 액세스 하는 스캔 방식
### 예시
부서가 10,20,30,40인 회사가 있는데 직원이 1000명인경우 각각 부서별로 500, 100, 200, 200명의  
직원이 있다. 우리가 알고싶은 내용이 급여가 5000~7000사이인 직원이다.  
생성된 인덱스는 (부서번호, 급여)로 결합된 인덱스가 있다.  
만약 Index Skip Scan을 사용하면 10번부서에서 5000~7000인 직원을 찾고 그 이후 10번 부서의 데이터는  
확인할 필요가 없기 때문에 20번 부서로 넘어가서 똑같이 반복한다.
### 작동 조건
```
일별업종별거래_PK: 업종유형코드 + 업종코드 + 기준일자
```
- 선두컬럼이 없을때만 작동하는것이 아님  
- 선두컬럼(업종유형코드)에 대한 조건절은 있고 중간컬럼(업종코드)에 대한 조건절이 없는 경우에도 Skip Scan을 사용할 수 있음.
## Index Fast Full Scan
논리적인 인덱스 트리 구조를 무시하고 인덱스 세그먼트 전체를 Multiblock I/O 방식으로 스캔하기 때문  
Index Fast Full Scan은 물리적인 순서로 블록을 스캔한다.  
Index Full Scan의 경우 논리적인 구조를 따라 루트 -> 브랜치1 -> 1, 2, 3, 4, 5 ...  
으로 읽어들인다.  
반면 Index Fast Full Scan은 물리적으로 디스크에 저장된 순서대로 인덱스 리프 블록들을 읽는다.  
- Mutiblock I/O의 특징인 대량의 블록을 읽어야 할때 큰 효과를 발휘함.
- 인덱스 리프 노드가 갖는 연결 리스트 구조를 무시한 채 데이터를 읽기 때문에 키 순서대로 정렬 X
- 인덱스가 파티션 돼 있지 않더라도 병렬 쿼리가 가능한것도 주요 특징중 하나 (병렬 쿼리시 Direct Path I/O 사용)

## Index Range Scan Descending
- 기본적으로 Index Range Scan과 동일한 스캔 방식
- 하지만 뒤에서 앞으로 읽기 때문에 정렬 결과는 반대 (내림차순)
- MAX값을 구하고자 할 때도 해당 컬럼에 인덱스가 있으면 인덱스를 뒤에서부터 한 건만 읽고 멈추는 실행계획이 자동수립 된다.